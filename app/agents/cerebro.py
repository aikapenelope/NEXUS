...
# Created on first use so env vars (API keys) are available at runtime,
# not at import time (avoids crashes during Docker build / testing).

_agents: dict[str, Agent[None, Any]] = {}


def _get_agent(name: str) -> Agent[None, Any]:
    """Return a Cerebro stage agent, creating it on first call."""
    if name not in _agents:
        configs: dict[str, dict[str, Any]] = {
            "research": {
                ...
                "instructions": (
                    "You are a research analyst. Given a topic or question, "
                    "gather all relevant facts, data points, and references. "
            ...
            "knowledge": {
                ...
                "instructions": (
                    "You are a knowledge organizer. Given raw research facts, "
                    "identify major themes, patterns, and connections. Produce "
                    "a coherent context paragraph that synthesizes the "
                    "information into a structured narrative."
                ),
            },
            "analysis": {
                "model": HAIKU_MODEL,
                ...
                "instructions": (
                    "You are a senior analyst. Given organized knowledge "
                    "about a topic, perform deep analysis: identify "
                    "non-obvious findings, assess risks and opportunities, "
                    "and assign a confidence score (0-1) based on evidence "
            ...
            "synthesis": {
                ...
                "instructions": (
                    "You are a synthesis expert. Given research findings, "
                    "organized knowledge, and deep analysis, produce a final "
                    "executive report with: a clear summary, top insights, "
                    "actionable recommendations, and an overall confidence "
                    "score. Include the intermediate outputs verbatim for "
                    "transparency."
                ),
            },
        }
...
async def run_cerebro(query: str) -> dict[str, Any]:
    ...
    research_result = await _get_agent("research").run(
    ...
    knowledge_result = await _get_agent("knowledge").run(
        knowledge_prompt,
        usage_limits=step_limit,
    )
    ...
    analysis_result = await _get_agent("analysis").run(
        analysis_prompt,
        usage_limits=step_limit,
    )
    ...
    synthesis_result = await _get_agent("synthesis").run(
        synthesis_prompt,
        usage_limits=step_limit,
    )
