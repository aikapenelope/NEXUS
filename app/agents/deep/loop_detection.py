...
Ported from vstorm CLI (cli/middleware/loop_detection.py).
Detects when an agent calls the same tool with identical arguments
repeatedly, and denies the call to force a different approach.

Critical for production coding agents that can get stuck in
fix-retry loops burning tokens indefinitely.
"""
